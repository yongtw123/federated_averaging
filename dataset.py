...
class SpeechCommandsDataset(Dataset):
    ...
    def __init__(self, paths, transform=None, db=None):
        """paths[Dict]: expects 
    ...
    def __loaditem(self, path, target):
        if self.db:
            ...
            # rstrip to remove newline char
            fullpath = os.path.join(self.dataset_root, os.path.normpath(path.rstrip()))
            samples, sr = librosa.load(fullpath, sr=16000)
